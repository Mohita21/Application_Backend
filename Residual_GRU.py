...
flags.DEFINE_integer("embedding_size", 1, "Dimension of data")
flags.DEFINE_float("missing_flag", -1.0, "missing value is marked with -1.0")

FLAGS = flags.FLAGS

variables_dict = {"W_imp": tf.Variable(tf.zeros([FLAGS.hidden_size, FLAGS.embedding_size]), name="W_imp"),
                  ...
                  "W_r": tf.Variable(tf.zeros([FLAGS.hidden_size], tf.float32), name="W_r")
                  ### Consider diagonal W_r, reducing the number of parameters for small data set (also used in NIPS2016
...
class LIMEGRU(object):
    def __init__(self, is_training, config, FLAGS):
        ...
        # residual = tf.nn.rnn_cell.ResidualWrapper(tf.nn.rnn_cell.BasicLSTMCell(size, forget_bias=0.0))
        residual = tf.nn.rnn_cell.ResidualWrapper(tf.nn.rnn_cell.GRUCell(size))
        cell = tf.nn.rnn_cell.MultiRNNCell([gru_cell, residual])
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(num_steps):
                ...
                if time_step == 0:
                    ...
                    self.F.append(cell_output + tf.multiply(self.F[time_step - 1], variables_dict[
                        "W_r"]))  ### Consider diagonal W_r for small data set
...
def main(_):
    ...
    miss_data, columns_name, norlizer = reader.read_raw_data("miss_data.csv")
    # raw_data, columns_name, norlizer = reader.read_raw_data("raw.txt")
    # miss_data = reader.read_missing_data("miss_data.txt", norlizer, FLAGS.embedding_size)
    ...
    with tf.Session(config=gpu_config) as session:
        ...
        with tf.variable_scope("LIMEGRU", reuse=None, initializer=initializer):
            model = LIMEGRU(is_training=True, config=config, FLAGS=FLAGS)  # train model,is_trainable=True
        ...
        for i in range(3):
            ...
            if i >= 5:
                print('RMSE:', reader.RMSE_Metric(miss_data,
                                                  np.concatenate((miss_data[0, :].reshape(1, -1), np.array(prediction)),
                                                                 axis=0), missing_flag=FLAGS.missing_flag))
                print('The Learning rate is:', new_lr)
            if i == 2:
                reader.save_data(miss_data, prediction, i, -1.0, norlizer, 'Residual')
            if i >= 10 and i % 10 == 0:
                if new_lr > 0.005:
                    new_lr = new_lr - 0.003
                else:
